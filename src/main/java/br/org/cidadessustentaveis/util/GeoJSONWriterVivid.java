...
public class GeoJSONWriterVivid {
	...
	public org.wololo.geojson.FeatureCollection write(List<Feature> features) {
		if(features == null) throw new IllegalArgumentException("Lista de features não pode ser nula");
		if(features.size() == 0) throw new IllegalStateException("Lista de features não pode ser vazia");

		org.wololo.geojson.Feature[] featuresJson = new org.wololo.geojson.Feature[features.size()];
		features.toArray(featuresJson);
	...
	public String convert(List<Feature> shapes) {
		return write(shapes).toString();
	}
}
